  ...
  fs.lstat(absolutePath, function(err, stats) {
    ...
    if (stats.isSymbolicLink()) {
      ...
      fs.stat(absolutePath, function(err2, stats2) {
        if (err2) {
          self._locked = false;
          return self._return(1, file, err2);
        }

        self.__stat(file, stats2);
        self._return(1, file);
      });

    } else {
      self.__stat(file, stats);
      self._return(1, file);
    }
  });
};
 ...
 */
FileEmitter.prototype.__stat = function(file, stats) {
  var obj;

  if (stats.isDirectory()) {
    this._locked = false;
    this._queues[2].push(file);

  } else if (stats.isFile() &&
      (!this.pattern || this.pattern.test(file))) {

    obj = new File(this, file, stats);
